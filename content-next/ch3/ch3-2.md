...
The condition uses `<=` instead of `<`. On the final iteration, `i` equals `numbers.Length`. For an array of size 5, that means `i` is 5, and there is no element at index 5. The shift goes past the end of the array, so the runtime throws an `IndexOutOfRangeException` and the program stops.
...
The loop starts `i` at 1 instead of 0, so index 0 is never visited and the first element is silently skipped. The program does not throw an error, which makes this bug harder to spot. Everything looks correct, but the output is missing a value.
...
The variable `position` holds 7, but valid indices for `data` are 0 through 4. The shift goes past the end of the array, and the runtime throws an `IndexOutOfRangeException`.

Whenever an index comes from a variable or a calculation rather than a literal number, the program cannot guarantee at compile time that the index is valid. The error only appears when the code runs with certain values: the code compiles without complaint but throws an error on certain inputs. Writing code to recover from a thrown error with `try` and `catch` is outside our current subset, so our goal here is to keep the index inside the valid range.
...
**Previous:** [Section 1 - Data and Memory](ch3-1.md)
  </div>
  <div style="text-align: right;">

**Next:** [Section 3 - Problem Solving with Arrays](ch3-3.md)
  </div>
</div>

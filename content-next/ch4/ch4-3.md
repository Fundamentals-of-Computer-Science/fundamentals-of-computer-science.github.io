...
### Throwing for Invalid State

Some methods have a meaningful answer for an empty list. `Count` returns 0. `Sum` returns 0. `Contains` returns false because no node contains the target value.

Other methods do not have an honest value to return. The maximum value of an empty list is not 0, because 0 might be a real value in a non-empty list. In that case, the method should signal that it cannot complete.
...
Description: "Max returns the largest value in the list. If the list is empty, Max throws an `InvalidOperationException` because there is no largest value to return."

The first `if` checks for an invalid state. If `this.head` is null, the method executes the `throw` statement and stops. If the list has at least one node, the method uses the same tracking pattern as the standalone maximum traversal.

We use `throw` here only as one-way error signaling. Writing code to handle the error with `try` and `catch` is outside our current subset.

---

**Try it yourself.**

Write a method named Min on LinkedList that returns the smallest value in the list. If the list is empty, throw an `InvalidOperationException` with a message that explains the problem.
...
The empty-list case comes first. After that check, the method can safely read `this.head.Value` and start the traversal from `this.head.Next`.
...
1. Write a method named Max on LinkedList that returns the largest value in the list. If the list is empty, throw an `InvalidOperationException`.
...
**Bug 2.** This method is supposed to count the nodes, but it has two problems. On a non-empty list, it returns one less than the correct count. On an empty list, it throws an error.
...
**Bug 3.** This method is supposed to append a value, but it throws an error when called on an empty list.
...
Next, we give the linked list configurable behavior in [Section 4 - Higher-Order Methods](ch4-4.md).

<div style="display: flex; justify-content: space-between; margin-top: 2rem;">
  <div style="text-align: left;">

**Previous:** [Section 2 - Basic Computation](ch4-2.md)
  </div>
  <div style="text-align: right;">

**Next:** [Section 4 - Higher-Order Methods](ch4-4.md)
  </div>
</div>

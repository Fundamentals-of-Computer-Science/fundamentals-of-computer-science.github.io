...
The higher-order methods from the previous section solved duplication where the changing part was the operation inside the traversal: an action, predicate, transform, or combine function. Here, the changing part is the type stored in the node and passed through the class. The next step is to make that type a parameter.
...
With `int`, that comparison is unambiguous. Two ints are equal when they hold the same numeric value. The trouble starts when `T` could be any type, because `==` means different things for different types.
...
ForEach and Filter work with values of type T but do not need to compare them or transform them into other types. Both keep the Chapter 3 and Chapter 4 higher-order vocabulary: ForEach accepts an action, and Filter accepts a predicate. The only new issue is that the element type is now `T`.
...
The generic Fold keeps the same recursive shape as the int-only version. A public Fold takes the initial value and the combine function, and delegates to a private Fold that takes a node, the accumulator, and the combine function. The base case is still the empty rest of the chain, where node is null. The recursive case still moves to the smaller list, `node.Next`, with an updated accumulator. Both methods carry `T` from the class and `U` from the method.
...
The recursive structure is the same as before: base case at null, smaller-input step through `node.Next`. The difference is that the accumulator is now `U`, and the node type is `LinkedListNode<T>?`. The method overloading pattern still works without any new rules.
...
Trace through this call. The chain holds the Rectangles in order 15×4, 3×8, 10×5 (head to tail, since Prepend adds at the front). Each line moves to the smaller rest of the chain and updates the accumulator; the trace stops at the base case, null:
...
Trace `accounts.Fold<double>(0.0, (acc, a) => acc + a.Balance)` on a list containing BankAccounts for Alice (500), Bob (1200), and Carol (350) in chain order from head to tail. Show each smaller-rest step until the trace reaches the base case.
...
The call style matches the int-only versions. The difference is that the values flowing through these methods are BankAccounts rather than ints.
...
Trace `shapes.Fold(0, (acc, r) => acc + r.Area())` where shapes contains Rectangles 4×3, 6×2, 5×5 (head to tail). Show the recursive unwinding, including each smaller-rest step and the base case at null, the same way we traced Fold earlier in this section.
...
3. Higher-order methods parameterized behavior: methods accepted behavior as input so the caller could decide what to do at each node. Generics parameterize type: `LinkedList<T>` accepts a type so the caller can decide what kind of data the list holds. Describe these two forms of abstraction in your own words, and explain how a method like `Map<U>` uses both at the same time.

4. The recursive int-only Fold used method overloading on `int` parameters. The generic Fold in this section uses method overloading on parameters that involve both `T` and `U`. Describe what changed between the two versions and what stayed the same. Your answer should mention the unchanged base case, the unchanged smaller-input step through `node.Next`, and why method overloading still works when type parameters are involved.

5. The generic Contains uses `.Equals()` instead of `==`. Explain why `==` does not work on an unconstrained type parameter, and describe what could go wrong when calling Contains on a `LinkedList<Rectangle>` if the Rectangle class does not override Equals.

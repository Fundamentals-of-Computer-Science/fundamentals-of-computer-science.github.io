...
If-else statements control behavior. Each branch can print to the console, rebind multiple variables, call methods for their effects, or do anything else. If the branches need to _do things_ rather than _produce a thing_, if-else is the right tool.
...
`int.Parse` takes a string and converts it to an integer. If the string contains a valid integer like "42" or "-7", the conversion succeeds and we get the number. If the user types something that is not a valid integer, like "hello" or "3.5", `int.Parse` throws a `FormatException` and the program stops. For now, we assume the user types valid integers. Handling errors with `try` and `catch` is outside our current subset.

This is one kind of thrown error. Another one appears in arithmetic: integer division by zero throws a `DivideByZeroException` because the operation cannot produce an integer result. A thrown error means the operation could not finish under the current conditions.
...
The division forms still follow the same rules as ordinary integer division. If the value after `/` or `%` is 0, the operation throws a `DivideByZeroException`.
...
### Wrapping Loops in Methods

The sum-from-1-to-n computation is useful enough to deserve a name. We can package it as a method.
...
Translation: "Define a method named SumTo that takes an integer n and returns the result of the scope."
...
The loop runs inside the method. The caller sees only the input and the result. This is the same method pattern from Chapter 1, applied to loops.

---

**Try it yourself.**

Write a method named `CountDivisible` that takes two integer parameters, `n` and `d`, and returns how many numbers from 1 to n are divisible by d.
...
Write a method named `CountDigits` that takes an integer n and returns how many digits it has. For example, `CountDigits(7364)` returns 4.
...
3. Write a method named `SumDigits` that takes an integer n and returns the sum of its digits.
...
You now know how to branch over numeric ranges, map values to results with switch expressions, count with loops, accumulate results, filter with modulo, search for values, and take numbers apart digit by digit. In the next section, we'll define our own numeric computations as methods.

<div style="display: flex; justify-content: space-between; margin-top: 2rem;"> <div style="text-align: left;">

**Previous:** [Section 2 - Computation](ch2-2.md)

</div> <div style="text-align: right;">

**Next:** [Section 4 - Methods](ch2-4.md)

...
For example, a weather app tracks the temperature for each day of the week. A gradebook needs a score for every student in the class. A game stores the position of every piece on the board. These problems all need to store multiple values of the same type _and_ work with those values as a group.

...
The array-based higher-order unit in Chapter 3 solved a similar problem by moving the changing part into a parameter. The traversal stayed the same, and the caller supplied an action, predicate, transform, or combine function. This section reuses that vocabulary rather than starting over: ForEach is for effects, Filter is for selection, Map is for transformation, and Fold is for reduction. The same idea transfers to LinkedList. Once those operations become methods on the list, variations like "count only values above 90" or "sum only the even values" become single calls instead of whole new methods.
...
When data is defined this way, the method that processes it can be defined the same way. Instead of writing "start at the head and walk," we can write "the answer for an empty list is one thing, and the answer for a non-empty list is built from the answer for the rest." The empty list is the base case. The rest of the chain, `node.Next`, is the smaller input. This is the same recursive pattern that defined Factorial, SumTo, and CountDigits: state a base case, then define the general case in terms of the same method applied to a smaller input. What changes here is the shape of the smaller version. Instead of a smaller integer, the smaller input is a shorter list.

A recursive version of Count says something different. The base case is the empty list: its count is 0. The recursive case is the non-empty list: its count is 1 plus the count of the rest of the list. The smaller-input step is following `node.Next`, which removes the current node from the problem.

To write this in C#, we need a method that takes a node and returns its count. The public Count method takes no node parameter because the list is self-contained. We could rewrite the public method to take a node, but callers would then have to reach into the list's internal structure and pass the head themselves, which defeats the encapsulation the class was built to provide.
...
Translation of the private method: "Define a method named Count that takes a nullable LinkedListNode and returns an int. The base case is node being null; return 0. The recursive case uses the smaller input node.Next; return 1 plus the result of calling Count with node.Next."
...
Translation of the private method: "Define a method named Count that takes a nullable LinkedListNode and returns an int. The base case is node being null, where the result is 0. The recursive case uses node.Next as the smaller list, where the result is 1 plus the result of calling Count with node.Next."

The switch expression form matches the recursive definition closely. An empty list is the base case and counts to 0. A non-empty list uses the smaller list after the current node and counts to 1 plus the count of the rest.
...
> **Recursive:** If node is null, the result is 0. Otherwise, the result is 1 plus the result of calling Count with node.Next, the smaller list after the current node.

The iterative version explains how to walk the list. The recursive version names the base case and explains how to build the answer from the smaller rest of the chain. They compute the same result, but they highlight different ways to think about the list.
...
Translation of the private method: "Define a method named Sum that takes a nullable LinkedListNode and returns an int. The base case is node being null, where the result is 0. The recursive case uses node.Next as the smaller list, where the result is node.Value plus the result of calling Sum with node.Next."
...
Private method: "Define a method named Contains that takes a nullable LinkedListNode and an integer value and returns a bool. The base case is node being null, where the result is false. The recursive case checks the current value, then uses node.Next as the smaller list if the value has not been found."

The recursive case uses short-circuit OR. If `node.Value == value`, the OR stops there and returns true without making the recursive call. If the values do not match, the OR evaluates the recursive call on `node.Next`, the smaller rest of the chain, and returns its result. Three logical cases (empty, found, keep searching) fold into two arms in the switch.
...
From here on, this section uses whichever form matches the problem more directly. Methods that perform an action at each step or build a new list as they go are written iteratively, because the loop follows the changing current reference and updates visible state. Methods that reduce a list to a single value in terms of the current node plus the smaller rest of the chain are written recursively. The exercises at the end ask you to rewrite several methods in the other form, so you practice both.
...
In the array higher-order unit, ForEach was a method that took an array and an action:
...
That version takes the array as its first parameter and the action as its second. At the call site, we pass both. The `Action<int>` notation and the `x => ...` lambda syntax are the Chapter 3 behavior-parameter notation; here we are only moving the traversal from arrays to linked lists.
...
Translation for `scores.ForEach(x => Console.WriteLine(x));`: "Call the ForEach method on the object referred to by scores, passing an action that takes an integer x and displays it."

The shift from array method to linked-list method is the same shift that turned earlier free-standing list operations into methods on LinkedList. The collection moves from inside the parentheses to the left of the dot, the parameter that held the collection becomes `this`, and the traversal changes from an index-based for loop to a reference-based while loop that follows node references. The higher-order idea stays the same: accept an action and apply it to each element.
...
Translation for `scores.Map(x => x + 10)`: "Call the Map method on the object referred to by scores, passing a transform that takes an integer x and returns x + 10. Bind the returned LinkedList to curved."
...
The definition of Fold on a linked list is itself recursive. The base case is the empty list: return the accumulator, because there is nothing left to combine. The recursive case is the non-empty list: combine the accumulator with the current value, then fold the rest of the chain. The smaller-input step is `node.Next`, the list after the current node. Each step absorbs one element into the accumulator and passes it forward to the next call.
...
Translation of the private method: "Define a method named Fold that takes a nullable LinkedListNode, an integer accumulator, and a combine function, and returns an int. The base case is node being null, where the result is acc. The recursive case uses node.Next as the smaller list and combine(acc, node.Value) as the updated accumulator."

The two arms of the switch expression match the two cases of the definition. An empty list is the base case and folds to whatever the accumulator currently holds. A non-empty list uses the smaller rest of the chain with an updated accumulator that already includes the current element.
...
Trace through the call the same way we traced recursive methods like Factorial and SumTo. The chain holds 85, 92, 78 (head to tail). The public Fold calls the private Fold with the head, 0, and the combine function:
...
Each call advances to node.Next, the smaller rest of the chain, and updates the accumulator. When node reaches null, the method has reached the base case and returns the accumulator unchanged.
...
Fold is tail-recursive. Each recursive case updates the node to the smaller rest of the chain and updates the accumulator, then hands off. Because nothing accumulates on the call stack across calls, a tail-recursive method can be rewritten as a loop in a mechanical way: the parameters of the recursive call become the updated values of the loop variables, and the recursive call itself becomes "go back to the top of the loop." The iterative Fold in the exercises below is exactly that conversion.
...
Sum and Count are specific instances of the general pattern that Fold captures.
...
Trace `data.Fold(0, (acc, x) => acc + 1)` on a list containing 10, 20, 30, 40. Show each smaller-input step until the trace reaches the base case, the way we traced the call above.
...
Write Fold iteratively. Your version should be a single method that takes an initial value and a combine function, starts an accumulator at the initial value, traverses the list with a while loop, and updates the accumulator at each node. This is a loop version of the same base-case/smaller-rest pattern: the loop stops at null and advances current to current.Next.
...
That reads inside out: identify the Filter call, wrap it in Fold, and sort out which arguments belong to which call. The method version reads left to right: start with scores, filter it, then fold the result.
...
4. What is a **method**?
5. What is **recursion**?
6. What is a **base case**?
7. What is a **recursive case**?
...
6. Write a recursive version of ForEach using method overloading. The public method takes an `Action<int>` and delegates to a private method that takes a nullable LinkedListNode and the action. Name the base case and the smaller-input step before writing the code.

7. Write a recursive version of Contains using method overloading and a switch expression. Name the base case and explain how `node.Next` is the smaller list.

8. Write a recursive method named `Max` that returns the largest value in a LinkedList. Use method overloading. For the base case where node is null, return `int.MinValue`. For the recursive case, compare node.Value to the result of calling Max on node.Next, the smaller rest of the chain, and return whichever is larger. You can use `Math.Max(a, b)` to get the larger of two integers.
...
1. Express the earlier Sum method as a Fold call. What initial value and combine function would produce the same result?
...
4. The array ForEach method and the linked-list ForEach method defined here compute the same thing. Describe the differences in terms of where the collection comes from, how the loop works, and what the call site looks like.

5. ForEach, Filter, and Map are written iteratively here, while Fold is written recursively. Explain why each choice fits the problem. Your answer should address what each method produces, where the base case would be, and how `node.Next` acts as the smaller input in the recursive versions.

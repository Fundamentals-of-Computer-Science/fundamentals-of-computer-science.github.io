...
That reads inside out: identify the Filter call, wrap it in Fold, and sort out which arguments belong to which function. The method version reads left to right: start with scores, filter it, then fold the result.
...
4. The standalone ForEach that took an array and the method ForEach defined here compute the same thing. Describe the differences in terms of where the collection comes from, how the loop works, and what the call site looks like.

5. ForEach, Filter, and Map are written iteratively here, while Fold is written recursively. Explain why each choice fits the problem. Your answer should address what each method produces and how that shapes which form reads more clearly.

...
What if you need to count only the values above 90? Or sum only the even values? Or check whether any value satisfies some condition other than equality? You could write separate methods for each variation: CountAbove, CountEven, SumPositive, ContainsOdd. But every one of those methods would contain the same traversal loop with a different operation inside.
...
We'll apply the same idea to LinkedList: rebuild Count, Sum, and Contains to work with any condition or operation, using the same patterns from Chapter 3. Once we have ForEach, Filter, Map, and Fold working on linked lists, we can combine them to answer complex questions without writing new methods.


### Recursive List Processing (Optional)

You can skip this section and return later if you prefer to focus on the higher-order methods first.
...
There's another way to think about list operations. A linked list node is either null (the empty list) or a value paired with a reference to another node. The structure refers to itself: a node's Next field holds a reference to another node of the same type. Every non-empty list is a value followed by a smaller list.

When data is defined this way, functions that operate on it can be defined the same way. Instead of writing "start at the head and walk," we can write "the answer for an empty list is one thing, and the answer for a non-empty list is built from the answer for the rest." That second case uses the answer to the same problem applied to a smaller version of the data. This is the recursive pattern from Chapter 2 with Factorial, SumTo, and CountDigits. What changes is the shape of the smaller version: instead of a smaller integer, the smaller input is a shorter list.
...
Display was one action performed on every element. What if we parameterize the action itself?
...
ForEach lets you act on every element. But what if you only want *some* elements?

Filter selects the elements that satisfy a condition and collects them into a new list. Contains checked for one specific value. Filter generalizes that idea: pass a test, and it keeps every value that passes.
...
Filter selects elements. ForEach performs actions. What if you want to transform every element into something new?
...
Sum and Count both accumulate a single value from the entire list. What's the general pattern?

Fold reduces a list down to a single value. It takes an initial value and a combine function, then builds one result from every element. Sum, Count, and finding the maximum are all instances of the same idea: start with something, then combine it with each element in turn.
...
Now we can answer complex questions without writing new methods.

Each method returns a LinkedList (or a value in Fold's case), so you can stack operations. Filter produces a list, and that list has Map, Filter, and Fold methods of its own. Chain them together with the dot operator:
...
That reads inside out: find Filter, wrap it in Fold, sort out which arguments belong to which call. The method version reads left to right: start with scores, filter it, fold the result.
...
The Chapter 3 higher-order methods now work on linked lists. The traversal changed from indices to references, but the main idea stayed the same: keep the traversal structure and let the caller supply the behavior.

Every method in this section operates on int values. The node stores an int. Filter's predicate takes an int. Map's transform takes an int and returns an int. What if you need a list that holds something other than integers?

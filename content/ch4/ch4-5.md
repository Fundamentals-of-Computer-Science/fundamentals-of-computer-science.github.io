...
3. Higher-order methods parameterized behavior: methods accepted functions as inputs so the caller could decide what to do at each node. Generics parameterize type: `LinkedList<T>` accepts a type so the caller can decide what kind of data the list holds. Describe these two forms of abstraction in your own words, and explain how a method like `Map<U>` uses both at the same time.
